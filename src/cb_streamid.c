...
struct std_cb_stream_table *creat_new_stream_instance(void)
{
	...
	//printf("%p is calloced", stream_entry_table_ptr);
	//printf("%p is calloced", stream_entry_ptr);
	return stream_entry_table_ptr;
}
void free_stream_memory(struct std_cb_stream_table *stream_table)
{
	...
	while (tmp_table) {
		...
		printf("%p is freed", tmp_table);
		printf("%p is freed", tmp_table->stream_ptr);
		...
		struct sr_qbv_conf *qbvconf)
{
	...
	if (!strcmp(nodename, "gate-enabled")) {
		...
		if (!qbvconf->basetime.nanoseconds) {
			snprintf(err_msg, MSG_MAX_LEN,
				 "The value of %s is zero", value->xpath);
			sr_set_error(session, err_msg, value->xpath);
